...
class SparseVectorStore(BaseVectorStore):
    ...
    def get_scores(self, query: str):
        ...
        for q in tokenized_query:
            ...
            "Your code here"
            word_freqs = np.array([0 if q not in self.doc_freqs[i] else self.doc_freqs[i][q] \
                          for i in range(self.corpus_size)])
            
            word_idf = self.idf[q] if q in self.idf else self._calculate_idf(1, self.corpus_size)

            cur_score = np.array(
                word_idf * word_freqs * (k1 + 1) / (word_freqs + k1 * (1 - b + b * np.array(self.doc_len) / self.avgdl))
            )
            score += cur_score
        return score
